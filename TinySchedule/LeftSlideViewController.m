...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        //tempAlpha2 0~kMainBgAlpha
        self.mainBgView.hidden = NO;
        CGFloat tempAlpha2 = kMainBgAlpha * (rec.view.frame.origin.x / (kScreenWidth - kMainPageDistance));
        self.mainBgView.alpha = tempAlpha2;
    }
    else
    {
        //超出范围，
...
- (void)openLeftView;
{
    ...
    self.mainBgView.hidden = NO;
    self.mainBgView.alpha = kMainBgAlpha;
