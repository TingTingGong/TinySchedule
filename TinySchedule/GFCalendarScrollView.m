...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            cell.todayLabel.textColor = SetColor(120, 120, 120, 1.0);
            
            // 标识今天
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear]) && indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                cell.todayLabel.textColor = AppMainColor;
            }
        }
        // 补上前后月的日期，淡色显示
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            cell.userInteractionEnabled = YES;
            cell.todayLabel.font = [UIFont systemFontOfSize:14.0];
            cell.todayLabel.text = [NSString stringWithFormat:@"%ld", indexPath.row - firstWeekday + 1];
            cell.todayLabel.textColor = SetColor(120, 120, 120, 1.0);
            
            BOOL isExitstToday = NO;
            NSString *todayYearMonthDay = [NSString stringWithFormat:@"%lu-%lu-%lu",(long)[[NSDate date] dateYear],(long)[[NSDate date] dateMonth],[[NSDate date] dateDay]];
            NSNumber *todayNumber = [StringManager stringDateTransferTimeStamp:todayYearMonthDay];
            // 标识选中的day或week
            NSDictionary *dict_date = [UserEntity getCalendarDate];
            if ([[dict_date objectForKey:@"state"] isEqualToString:@"0"]) {
                
                NSString *string = [NSString stringWithFormat:@"%lu-%lu-%@",(long)monthInfo.year,(long)monthInfo.month,cell.todayLabel.text];
                NSNumber *number = [StringManager stringDateTransferTimeStamp:string];
                if ([number longLongValue] >= [[dict_date objectForKey:@"number1"] longLongValue] && [number longLongValue] <= [[dict_date objectForKey:@"number2"] longLongValue]) {
                    cell.todayCircle.backgroundColor = AppMainColor;
                ...
                if ([todayNumber longLongValue] >= [[dict_date objectForKey:@"number1"] longLongValue] && [todayNumber longLongValue] <= [[dict_date objectForKey:@"number2"] longLongValue]) {
                    isExitstToday = YES;
                }
            }
            else
            {
                if ((monthInfo.month == [[dict_date objectForKey:@"date"] dateMonth]) && (monthInfo.year == [[dict_date objectForKey:@"date"] dateYear])) {
                    if (indexPath.row == [[dict_date objectForKey:@"date"] dateDay] + firstWeekday - 1) {
                        cell.todayCircle.backgroundColor = AppMainColor;
                        cell.todayLabel.textColor = [UIColor whiteColor];
                    }
                    else {
                    ...
                    if ([todayNumber longLongValue] == [[dict_date objectForKey:@"number1"] longLongValue]) {
                        isExitstToday = YES;
                    }
                }
                else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.didSelectDayHandler != nil) {
        ...
        NSDictionary *dict_date = [UserEntity getCalendarDate];
        if ([[dict_date objectForKey:@"state"] isEqualToString:@"0"]) {
            [StringManager setCalendarWeekDictionary:mydate];
        }
        else
        {
